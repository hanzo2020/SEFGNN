...
def set_seed(random_seed=2025):
    ...
    # torch.use_deterministic_algorithms(True) #显存暴增
...
class MultiPPI_Encoder():
    ...
    def drop_samples(self, fold, sample_neg=0., sample_pos=1., num_samples=0):
        ...
        splitted_idx = self.dataset.get_idx_split(fold) if len(
            self.args['PPI']) == 5 else self.ori_dataset.get_idx_split(fold)
    ...
    def init_model(self):
        ...
        elif 'SEFMGNN' == args['model']:
            model = self.model = SEFMGNN(
                in_channels=self.num_node_features,
                hidden_channels=64,
                num_ppi=len(self.dataset),
                residual=args['residual'],
    ...
    def train_epoch(self, epoch):
        self.model.train()
        gene_names = np.genfromtxt('data/Gene-Name.txt', delimiter='\n', dtype=str, skip_header=1)
        tot_loss, steps = 0, 0
        constraint_loss = 0
        for data_tuple in zip(*self.train_loader_list):
            ...
            elif isinstance(self.model, SEFMGNN):
                out, x_list, ppi_weight, constraint_dict = self.model(data_tuple)
            else:
            ...
            if isinstance(self.model, Multi_GTN):
                ...
                loss = constraint_dict['ce_loss'] + constraint_dict['constraint_loss'] * 1e-4
            else:
        ...
        for data_tuple in zip(*self.train_loader_list):
            ...
            with torch.no_grad():
                if isinstance(self.model, Multi_GTN):
                    ...
                    out, _, _, _ = self.model(data_tuple)
                else:
                    out = self.model(data_tuple)
        ...
        for data_tuple in zip(*self.valid_loader_list):
            ...
            y_true = np.append(y_true, true_lab.cpu().detach().numpy())
            if isinstance(self.model, Multi_GTN):
                valid_loss += self.loss_func(out, true_lab.float(), x_list, ppi_weight.to(self.args['gpu']),
                                             epoch).item()
            elif isinstance(self.model, SEFMGNN):
                valid_loss += constraint_dict['ce_loss'] + constraint_dict['constraint_loss'] * 1e-4
            else:
        ...
        for data_tuple in zip(*self.test_loader_list):
            ...
            names = gene_names[index.cpu().detach().numpy()]
            true_lab = data_tuple[0].y[:size][:, 1]
            out = out.view(-1)
            pred_lab = np.zeros(size)
            if size == 1:
            ...
            if isinstance(self.model, Multi_GTN):
                ...
                test_loss += constraint_dict['ce_loss'] + constraint_dict['constraint_loss'] * 1e-4
            else:
                loss = self.loss_func(out, true_lab.float()).item()
        ...
        if isinstance(self.model, SEFMGNN):
            print('alpha: ' + str(constraint_dict['alpha']))
        out_dict = {
            'train_loss': tot_loss,
